...
# Sequences deserve their own pool. Without it, a parameter called `seq` falls through to
# the untyped pool harvested from the repo's tests and gets handed a bare `0` - so the only
# disagreement found is `TypeError: 'int' object is not iterable` versus some other error,
# which is a real difference and useless as evidence.
SEQ_CORNERS = [
    "[1, 2, 3]",
    "[]",
    "[0]",
...
def _looks_like_sequence(name: str) -> bool:
    n = name.lower()
    return n in {"seq", "seqs", "xs", "ys", "items", "values", "keys", "iterable", "it"} or any(
        k in n for k in ("seq", "list", "items", "iterable", "elements", "coll")
    )
...
def for_parameter(name: str, annotation: str | None, harvested: dict[str, list[str]]) -> list[str]:
    ...
    # A sequence parameter goes first, and ahead of the harvested pool. A bare literal
    # scraped from the repo's tests is often a scalar, and handing one to a function that
    # iterates its argument produces `TypeError: not iterable` on both sides - which
    # cannot distinguish anything, so the run establishes nothing.
    if _looks_like_sequence(name) or any(
        k in ann for k in ("list", "sequence", "iterable", "tuple", "set", "dict", "collection")
    ):
        out.extend(SEQ_CORNERS)
